...
const AllDonationRequest = () => {
  ...
  const donationRequest = allRequest.filter(
    (req) => req.requesterEmail !== userEmail
  );

  const handelRequestDone = (id, Status) => {
    axiosSecure.patch(`/donationRequest/${id}`, { Status }).then((res) => {
      if (res.data.modifiedCount > 0) {
        refetch();
        const donerInformation = { donerName, userEmail };
        axiosSecure.post("/doners", donerInformation).then((res) => {
        ...
        <Table.Body className="divide-y divide-gray-25">
          {donationRequest.map((card) => (
            <>
              <Table.Row
                key={card._id}
                className="bg-white dark:bg-zinc-500 dark:text-white"
              >
                <Table.Cell>
                ...
                <Table.Cell>
                  {card.Status === "Pending" ? (
                    <div className="flex gap-2">
                      <Button
                        onClick={() =>
                          handelRequestDone(card._id, "Inprogress to Done")
                        }
                        className="border border-green-600 dark:bg-slate-500"
                        size="md"
                        type="outlinePrimary"
                      >
                        Done
                      ...
                      <Button
                        onClick={() =>
                          handelRequestDone(card._id, "Inprogress to Cancel")
                        }
                        className="border border-red-600 dark:bg-slate-500"
                        size="md"
                        type="outlinePrimary"
                      >
                        Canceled
